...
export async function checkBSOR(file, isLink) {
    if (isLink) {
        ...
        if (filename.split('.').pop() === 'bsor' || filename.split('.').pop() === 'bsortemp') {
            const replayRes = await axiosInstance.get(file, {
                responseType: "blob"
            })
            return await checkBSORFile(replayRes.data)
        } else {
            return 'Error: wrong file format';
        }
    } else {
        return await checkBSORFile(file);
    }
}

async function checkBSORFile(file, completion) {
    const arrayBuff = await file.arrayBuffer();
    return decode(arrayBuff, completion);
}
